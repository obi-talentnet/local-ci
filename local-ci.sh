...
echo
if [ $choice = "js" ]; then
    wget https://raw.githubusercontent.com/obi-talentnet/local-ci/master/tn-hooks/pre-commit-js.sample 
    mv pre-commit-js.sample  .git/hooks/pre-commit 
elif [ $choice = "php" ]; then
    wget https://raw.githubusercontent.com/obi-talentnet/local-ci/master/tn-hooks/pre-commit-php.sample 
